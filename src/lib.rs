...
unsafe impl<L: DefaultLinkOps, D: ?Sized> intrusive_collections::linked_list::LinkedListOps
    for LinkedListDynLinkOps<L, D>
where
    L::Ops: intrusive_collections::LinkOps<LinkPtr = NonNull<L>>
        + intrusive_collections::linked_list::LinkedListOps,
{
    #[inline]
    unsafe fn next(&self, ptr: Self::LinkPtr) -> Option<Self::LinkPtr> {
        self.ops
    ...
    unsafe fn prev(&self, ptr: Self::LinkPtr) -> Option<Self::LinkPtr> {
        self.ops
            .prev(LinkedListDynLink::to_link(ptr))
            .map(LinkedListDynLink::from_link)
    }
